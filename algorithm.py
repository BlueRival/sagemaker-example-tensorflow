...
#
# This template provides all the structure to deploy any Python based learning algorithm to SageMaker. You will only need
# to provide this algorithm module with three functions exported in order to operationalize your algorithm.
#
...
# work with will also have these mappings built for you. The difference is that in development S3 is not used. In development
# there is a directory in the source code called opt. The scripts in this template will wire up the directories/files in the following
# manner:
#
...
# Interface
#
# Your algorithm in this file (Python module) must export three functions. You may have other functions in this module or
# in other modules, but the template in this project will ignore all of that. The functions you must export and implement
# are Init(), Train() and Predict(). Here are descriptions of what each function should do.
#
# Init(mode):
#
# Input: Mode is a string either 'train' or 'predict'. This allows you to setup your algorithm. For this example algorithm
#         I was able to use a single init that works regardless of train or predict mode. You may need to init your module
#         differently depending on the mode of operation. If you can have one init for both modes, like this template, simply
#         make Init() an empty function as is shown below.
#
...
# Predict(features):
#
# Input: Takes a Python Dictionary object of all input features for a single example to classify/predict/etc. Each feature
#         on the example will be identified by name.
...
#
#
# Note: For AWS notes on Docker file paths mentioned above, see https://docs.aws.amazon.com/sagemaker/latest/dg/your-algorithms-training-algo.html
...
classifier = tf.estimator.DNNRegressor(
  ...
  hidden_units=[20, 20],  # Two hidden layers of 20 nodes each.
...
def Train():
  ...
  # Train the Model.
  classifier.train(input_fn=lambda: get_training_dataset(train_features, train_labels), steps=1000)
